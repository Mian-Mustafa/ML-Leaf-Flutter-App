...
/// Content testing (§21) for Module 9 (Ensemble Methods): validates the
/// bundled lesson JSON parses, is well-formed, and that every referenced figure
/// asset is loadable.
void main() {
  TestWidgetsFlutterBinding.ensureInitialized();

  test('ensemble_methods lessons load, ordered, unique ids', () async {
    final repo = LessonRepository(loader: rootBundle.loadString);
    final lessons = await repo.loadForModule('ensemble_methods');
    ...
    for (final l in lessons) {
      expect(l.moduleId, 'ensemble_methods');
      expect(l.title.trim(), isNotEmpty);
    }
  });

  test('every figure referenced in Module 9 exists as an asset', () async {
    final repo = LessonRepository(loader: rootBundle.loadString);
    final lessons = await repo.loadForModule('ensemble_methods');
